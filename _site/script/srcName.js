...
const getByName = document.getElementById("srcName").addEventListener("click",
    function getValue() {
        const nameResult = document.querySelector('#displayResult')
        const getInputValue = document.getElementById("inputForName").value;
        if (getInputValue === '') {
            console.log('enter a  drink')
            nameResult.outerHTML = ' <div id="displayResult"><spam class="helpText">please, enter a valid cocktail name</spam></div>'
        } else {
            nameResult.outerHTML = ' <div id="displayResult"></div>'
            nameRequest(srcName + getValue);
...
// define api url
const srcName = 'https://www.thecocktaildb.com/api/json/v1/1/search.php?s=';
...
const nameRequest = (url) => {
    fetch(url)
        ...
        .then((data) => {
            ...
            //target node
            const printResult = document.querySelector('#displayResult');
            ...
            const liItems = document.querySelectorAll('#byName ul li')
            for (let i = 0; i < liItems.length; i++) {
                let givesValue = liItems[i].innerHTML;
                let givesClass = liItems[i].classList;
